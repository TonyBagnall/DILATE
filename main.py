...
def main():
    ...
    for ind in range(1, 51):
        ...
        for net in nets:
            ...
            input_series = test_inputs.detach().cpu().numpy()[ind, :, :]
            target = test_targets.detach().cpu().numpy()[ind, :, :]
            preds = pred.detach().cpu().numpy()[ind, :, :]

            plt.subplot(1, 3, k)
            plt.plot(range(0, N_input), input_series, label="input", linewidth=3)
            plt.plot(
                range(N_input - 1, N_input + N_output),
                np.concatenate([input_series[N_input - 1:N_input], target]),
                label="target",
                linewidth=3,
            )
            plt.plot(
                range(N_input - 1, N_input + N_output),
                np.concatenate([input_series[N_input - 1:N_input], preds]),
                label="prediction",
                linewidth=3,
            )
